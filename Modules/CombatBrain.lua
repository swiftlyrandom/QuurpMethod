-- CombatBrain.lua – retreat → weave approach → high‑orbit 2
local Players = game:GetService("Players")
local player = Players.LocalPlayer

local CombatBrain = {}
local LOCK_RANGE          = 1600
...
local WEAVE_AMPLITUDE     = 500   -- or even 300
...
function CombatBrain.update(body, dt)
    ...
    local heightFraction = (myPos.Y - enemyPos.Y) / ORBIT_ALTITUDE_OFFSET  -- 0..1
    ...
    -- ---- Weave timer (shared by retreat & approach) ----
    ...
    if phase == "retreat" then
        -- Fly away from enemy, climb, lateral weave
    ...
    elseif phase == "approach" then
        -- Fly toward the orbit centre, lateral weave
        local orbitCenter = Vector3.new(enemyPos.X, targetOrbitY, enemyPos.Z)
        ...
        -- The target is the orbit centre shifted sideways – changes every weave interval
